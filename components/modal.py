...
class Modal:
    def __init__(self, screen, guest):
        ...
        # Set initial position based on the guest's position
    ...
    def set_initial_position(self):
        """Set the modal's position below the guest."""
        guest_center_x = self.guest.x
        guest_center_y = self.guest.y

        # Adjust the vertical offset to make the modal even lower
        vertical_offset = 100  # Increase this value to make the modal lower
        self.x = int(guest_center_x - self.width // 2)
        self.y = int(guest_center_y + vertical_offset)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.x + self.width - 40 <= mouse_x <= self.x + self.width - 10 and \
               self.y + 10 <= mouse_y <= self.y + 40:
                ...
                selected_guest = None  # Clear selection when modal is closed
                return "close"
